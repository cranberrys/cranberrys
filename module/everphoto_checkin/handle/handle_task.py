...
from ac_api import ac_template, AcResponse, AcView
from .everphoto_api import TaskManager


@ac_template('task_add.jinja2')
...
async def ec_task_smscode(request):
    ...
    if resp['code'] != 0:
        return AcResponse.json({'code': -1, 'msg': resp['message'], })
    return AcResponse.json({'code': 0, 'msg': '发送成功'})


class ECView(AcView):
    @ac_template('task_list.jinja2')
    async def get(self):
        with self.app.data_manager('task_list') as task_list:
    ...
    async def post(self):
        ...
        resp = await self.app['ec_api'].login(mobile, smscode)
        if resp['code'] != 0:
            return AcResponse.json({'code': -1, 'msg': resp['message'], })
        ...
        return AcResponse.json({'code': 0, 'msg': '添加成功', })
    ...
    async def delete(self):
        ...
        return AcResponse.json({'code': 0, 'msg': '删除成功', })


class ConfigView(AcView):
    @ac_template('config.jinja2')
    async def get(self):
        with self.app.data_manager('cron_job') as cron_job:
            edit_job = cron_job
    ...
    async def post(self):
        ...
        if not config_data:
            return AcResponse.json({'code': -1, 'msg': '配置为空', })
        try:
            edit_job = json.loads(config_data)
        except JSONDecodeError:
            return AcResponse.json({'code': -1, 'msg': '配置格式有误', })
        self.app.cron_job.edit(key='auto_check_in', **edit_job)
        with self.app.data_manager('cron_job') as cron_job:
            cron_job.clear()
            cron_job.update(edit_job)
        return AcResponse.json({'code': 0, 'msg': '保存成功', })
